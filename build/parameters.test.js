...
import { checkControlPlaneGroupMembershipParameters, getControlPlaneParameters, listConsumersParameters, listControlPlaneGroupMembershipsParameters, listPluginsParameters, listRoutesParameters, listServicesParameters } from "./parameters.js";
...
test("validated control plane schema accepts a UUID-shaped id for all relevant tools", () => {
    ...
    assert.equal(listControlPlaneGroupMembershipsParameters().parse({ groupId: VALID_ID }).groupId, VALID_ID);
    assert.equal(checkControlPlaneGroupMembershipParameters().parse({ controlPlaneId: VALID_ID }).controlPlaneId, VALID_ID);
});
test("validated control plane schema rejects traversal input", () => {
    ...
    assert.equal(listControlPlaneGroupMembershipsParameters().safeParse({ groupId: "../api-products" }).success, false);
});
