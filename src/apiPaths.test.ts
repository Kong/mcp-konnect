...
test("buildControlPlaneGroupMembershipsEndpoint keeps a normal pageAfter value as one query parameter", () => {
  ...
  assert.equal(
    endpoint,
    `/control-planes/${VALID_ID}/group-memberships?page[size]=25&page[after]=cursor-123`
  );
});
...
test("buildControlPlaneGroupMembershipsEndpoint encodes a fragment-like pageAfter value", () => {
  ...
  assert.equal(
    endpoint,
    `/control-planes/${VALID_ID}/group-memberships?page[size]=25&page[after]=cursor%23fragment`
  );
});
...
test("buildControlPlaneCoreEntitiesListEndpoint keeps a normal offset as one query parameter", () => {
  ...
  assert.equal(
    endpoint,
    `/control-planes/${VALID_ID}/core-entities/services?size=50&offset=cursor-123`
  );
});
...
test("buildControlPlaneCoreEntitiesListEndpoint encodes an offset containing ampersands", () => {
  ...
  assert.equal(
    endpoint,
    `/control-planes/${VALID_ID}/core-entities/routes?size=50&offset=foo%26size%3D1000`
  );
...
test("buildControlPlaneCoreEntitiesListEndpoint encodes spaces and equals in offset values", () => {
  ...
  assert.equal(
    endpoint,
    `/control-planes/${VALID_ID}/core-entities/plugins?size=10&offset=cursor%20one%3Dtwo`
  );
});
